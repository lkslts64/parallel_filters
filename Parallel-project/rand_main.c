...
;int main (int argc, char* argv[])
{
  ...
  MPI_Comm_size (MPI_COMM_WORLD, &size);        /* get number of processes */
	...
	printf( "Hello world from process %d of %d . north=%d,south=%d,east=%d,west=%d\n", rank, numprocs,north,south,east,west );
	//two arrays for storing image
	int arrsizeI=10;
	int arrsizeJ=10;
	unsigned char *arr1=malloc(arrsizeI*arrsizeJ*sizeof(unsigned char));		
	unsigned char *arr2=malloc(arrsizeI*arrsizeJ*sizeof(unsigned char));
	...
	//upleft=arr1[0];upright=arr1[arrsizeJ];botleft=arr1[arrsizeI*arrsizeJ-10];botright=arr1[arrsize*arrsize-1];		//the corners to be sent
	...
	//double filter[9];
	double filter[9]={3,0,2,0,1,0,1,0,4};
	sum=11;
	/*for(i=0;i<9;i++){
		filter[i]=rand()%4;
		sum+=filter[i];
	...
	int result;
		...
		/*//printf("START\n");
		MPI_Isend(arr1,1,row,north,0,MPI_COMM_WORLD,&req[0]);		//sending upper-row to northern process.
		MPI_Isend(arr1+90,1,row,south,0,MPI_COMM_WORLD,&req[1]);	//sending bottom-row to southern process.
		MPI_Isend(arr1+9, 1, column,east,0, MPI_COMM_WORLD,&req[2]); //sending right-column to east
		MPI_Isend(arr1, 1, column, west, 0, MPI_COMM_WORLD,&req[3]);	//sending left column to west
		MPI_Isend(&arr1[0],1,MPI_UNSIGNED_CHAR,upleftdiag,0,MPI_COMM_WORLD,&req[4]); 	//sending upper-left-diagonal element (one)
		MPI_Isend(&arr1[arrsizeJ],1,MPI_UNSIGNED_CHAR,uprightdiag,0,MPI_COMM_WORLD,&req[5]);			//single element diagonal
		MPI_Isend(&arr1[arrsizeI*arrsizeJ-10],1,MPI_UNSIGNED_CHAR,botleftdiag,0,MPI_COMM_WORLD,&req[6]);	// 	 ^^		^^		^^
		MPI_Isend(&arr1[arrsizeI*arrsizeJ-1],1,MPI_UNSIGNED_CHAR,botrightdiag,0,MPI_COMM_WORLD,&req[7]);	// 	 ^^		^^		^^
		//receive
		MPI_Irecv(Nrow,arrsizeJ, MPI_UNSIGNED_CHAR,north, 0, MPI_COMM_WORLD,&req[8]);
		//MPI_Irecv(Nrow,1, row,north, 0, MPI_COMM_WORLD,&req[8]);
		MPI_Irecv(Srow,arrsizeJ, MPI_UNSIGNED_CHAR,south, 0, MPI_COMM_WORLD,&req[9]);
		MPI_Irecv(Ecol,arrsizeI, MPI_UNSIGNED_CHAR,east, 0, MPI_COMM_WORLD,&req[10]);
		MPI_Irecv(Wcol,arrsizeI, MPI_UNSIGNED_CHAR,west, 0, MPI_COMM_WORLD,&req[11]);
		MPI_Irecv(&upleft,1,MPI_UNSIGNED_CHAR,upleftdiag,0,MPI_COMM_WORLD,&req[12]);
		MPI_Irecv(&upright,1,MPI_UNSIGNED_CHAR,uprightdiag,0,MPI_COMM_WORLD,&req[13]);
		MPI_Irecv(&botleft,1,MPI_UNSIGNED_CHAR,botleftdiag,0,MPI_COMM_WORLD,&req[14]);
		MPI_Irecv(&botright,1,MPI_UNSIGNED_CHAR,botrightdiag,0,MPI_COMM_WORLD,&req[15]);*/
